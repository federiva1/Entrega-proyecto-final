...
const modelosPorMarca = {
    'BETA': ['QM 200', 'RT 250 ST', 'PANDA 70 4T', 'ZONTES R 310', 'ARROW 150'],
    'BAJAJ': ['DOMINAR 400', 'ROUSER 200 NS', 'AVENGER STREET 220', 'DISCOVER 125', 'BOXER RT 150'],
    'BMW': ['S 1000 RR', 'F 800 R', 'R 1200 GS', 'G 450 X', 'K75'],
    'KAWASAKI': ['ER6N', 'NINJA 300', 'KVF 150', 'ZX 10 R (NINJA)', 'KLE 650 VERSYS'],
    'KELLER': ['KR 250', 'CLASSIC FULL', 'CRONO', 'KN 150', 'MIRACLE']
};
...
`
    ...
    fetch("/Productos/ProductosMoto.json")
        .then(response => response.json())
        .then(data => {
            productosMoto = data;

            for (const [index, producto] of productosMoto.entries()) {
                if (producto.Antiguedad <= edad.value) {
                    ...
                    button.addEventListener("click", (event) => {
                        ...
                        const productoContratado = productosMoto[index];
                        Contratar(productoContratado, precio); //
                    });
...
let Datos_IdentificacionVehiculo = [];
let DatosCobertura = [];

function Contratar(productoContratado, precio) {
    ...
    productoContratado.cuota = parseFloat(productoContratado.Coeficiente) * precio.value;

    DatosCobertura.push(productoContratado);
    localStorage.setItem("DatosCobertura", JSON.stringify(DatosCobertura));
    ...
    function validarDatos() {
        ...
        let DatosEspecificos = { Patente: Patente.value, Chasis: Chasis.value, Motor: Motor.value };

        Datos_IdentificacionVehiculo.push(DatosEspecificos);

        let arreglos_JSON = JSON.stringify(DatosEspecificos);

        localStorage.setItem("Datos_IdentificacionVehiculo", arreglos_JSON);
        ...
        btnFinalizar.addEventListener("click", () => {
            ...
            let MDPElegido = MedioDePago.value;
            mediosDePago.push(MDPElegido);
            localStorage.setItem('mediosDePago', JSON.stringify(mediosDePago));
            ...
            if (MedioDePago.value === "cbu" && !cbu.value) {
                ...
                return;
            } else if (MedioDePago.value === "tarjeta" && (!nombre_titular.value || !numero_tarjeta.value || !fecha_vencimiento.value || !codigo_seguridad.value)) {
                Toastify({
                    text: "Faltan completar datos",
                    backgroundColor: "red"
                }).showToast();
...
//Guardar datos medio de pago//
            ...
            setTimeout(function () {
                window.location.href = "FinalMoto.html";
            }, 3000);
        });
    }
